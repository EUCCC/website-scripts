...
/* Modification log:
 * -------------------------------------------------------------------------
 * 2015/05/21	bb	1.5	don't pass $db to functions, add comments
...
echo <<<EOS
<h2>   Admin Gold Card Form v1.5  </h2>
Create a new Gold Card and change member status<br/>
Instructions
<ul>
<li>Enter values and click Submit</li>
<li>This will create a <em>new</em> Gold Card, which will replace a Gold Card 
		entered by the member</li>
<li>Use Admin Member Update to change only status, e.g., "Gold Card Pending" 
		to "Gold Card"</li>
</ul>
EOS;
...
$blocksize = 30;
//Joomla! group name
define("EU_BOARD_GROUP", "EU Board of Directors"); 
//Joomla! group name
define("EU_MEMBER_DATABASE_ADMIN", "EU Member Database Administrators"); 
 ...
 */ 
function buildAndExecuteSearchQuery($blocksize)
{
    $db = \JFactory::getDBO();
...
function displayManyMembersTable($members)
{
    ...
    echo '<tr style="background-color:#EBEBEB;"><th>Select</th>' .
    '<th>First Name</th>' .
    ...
    echo '&nbsp<INPUT name="back" type="button" value="Back" ' .
 ...
 * @return void
 */ 
function displayNewGoldcardForm($member)
{
    ...
    // status 
    insertPulldownMenu(
        'Status', 'status', $_SESSION['sid'], 
        $_SESSION['sdesc'], '', ''
    );

    echo '<tr><td class="blabel">May we contact you?</td>' . 
    '<td>Yes <input type="radio" checked="checked" ' .
    'name="alumni_event_contact" value=1>No<input type="radio" ' .
    'name="alumni_event_contact" value=0></td></tr>';
 ...
 * @return void
 */ 
function updateJoomlaUsersTable()
{
 ...
 * @return void
 */ 
function dropFromJoomlaGroup($group_name)
{
    $db = \JFactory::getDBO();
 ...
 * @return void
 */ 
function closeOldBoardPositions()
{
    $db = \JFactory::getDBO();
    $user_id_to_edit = trim($_SESSION['member_id']);
    $query = $db->getQuery(true);
    ...
    dropFromJoomlaGroup(EU_BOARD_GROUP);
    return;
}
// --------------------------------------------------------------------
/**
...
{
    dropFromJoomlaGroup(EU_MEMBER_DATABASE_ADMIN);
}
// --------------------------------------------------------------------
/**
...
function updateDatabaseTables()
{
    ...
    try {
        ...
        if ($e->getCode()==UNIQUE_CONSTRAINT_ERROR) {        
            ...
            echo "<br/><strong>Duplicate Gold Card entry -- 
				database was not updated</strong><br/>
				(New Gold Card can be entered for this member tomorrow, 
				$tomorrow, or later)<br/><br/>";
        } else {
            echo "<br/>" . $e->getMessage() . "<br/><strong>-- database 
				was not updated --</strong><br/>";
        }
    }
    return $success;
}
...
if (!isset($_POST['process'] )) {
    ...
    $members = buildAndExecuteSearchQuery($blocksize);
    displayManyMembersTable($members);
} elseif ($_POST['process'] == 2) {
    if (isset($_POST['member_radio'])) {
        $member = buildAndExecuteMemberQuery();
        displayNewGoldcardForm($member);
    } else {
        echo "<br/><strong>Please select a member to Gold-Card.
			</strong><br/><br/><br/>";
        echo '<input name="back" type="button" value="Back" ' .
