<!------------------------------------------------------------------------
 ...
 2014/08/17   js   v1.0 Changed to use JDatabase function, removed db_connect to avoid hard
 		coding database name and password.
...
for ($i = 1; $i <= $months_to_check; $i++) {
    ...
    $query = "select count(*) from eu_member_hours where task_id=22
        and year(task_date) = year(date_add(last_day(curdate()), interval -$i month))
        and month(task_date) = month(date_add(last_day(curdate()), interval -$i month))";
	$db->setQuery($query);
	$total = $db->loadResult();
