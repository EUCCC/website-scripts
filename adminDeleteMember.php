 ...
 * @version   1.3
 * @link      https://github.com/EUCCC/website-scripts/blob/master/adminDeleteMember.php
 */
 
/*
 * Modification log:
 *  -------------------------------------------------------------------------
 * 2015/05/20    bb    1.3    don't pass $db to functions, add comments
...
echo <<<EOS
<h2>Admin Delete Member v1.3</h2>
Instructions
<ul>
	<li>Active and Gold-Card-Pending status members cannot be deleted
		<ul><li>Change Active or Gold-Card-Pending member to an inactive 
		status (Inactive, Gold Card, etc.) using Admin Member Update, then 
		delete</li></ul>
	</li>
</ul>
<br/>
...
// ------------------------------------------------------------------------
/**
 * Show data for member to be deleted
 * 
 * @param object $member object with member data
 * 
 * @return void
 */ 
function showSingleMemberData($member)
{
    // Display column headings
    echo "<table border=1>";
 ...
 * Build and execute query to get member information from database
 * 
 * @param int $member_id EU member ID
 * 
 * @return void
 */ 
function buildAndExecuteSingleMemberQuery($member_id)
{
    $db = \JFactory::getDBO();
...
// ------------------------------------------------------------------------
/**
 * Update eu_members table and joomla users table -- 
 *    prefix member email address/login with "deleted" and a random integer,
 *    Changes email address to "deleted+NN+oldEmail" in eu_members table and in
 *    Joomla! users database, where NN is a random #, and changes member status 
 *    to "D" in eu_members table
 *    
...
if (!isset($_POST['process'] )) { 
    ...
    if (!isset($_POST['member_radio'])) {
        ...
        $member_data = buildAndExecuteSingleMemberQuery($member_id);
        $_SESSION['member_id'] = $member_id;
        $_SESSION['email_address'] = $member_data->email_address;
        showSingleMemberData($member_data);
    }
