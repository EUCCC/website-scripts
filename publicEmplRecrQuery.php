 ...
 * 
 * 
 * @package    employer-recruiter-scgb.php
 * @author     Roger Laurel
 ...
 * 1.3 bb -- 042815 -- added Back button, reorder name in table, rework control 
 * 						logic, restyle table
 * 1.2 bhb -- added name and email to match in query, don't match members with 
 * 						empty profile (was empty desired_position),
 *     					and added </form> tag, which had been missing
 * 1.1 bhb -- added button for linkedin URLs and http:// to URLs (if needed) 
 * 						so apache URL rewrite would not make URLs local links
 */
echo <<<EOS
<h2>Employer/Recruiter Query v1.3</h2>
EOS;
...
function load_search_parms()
{
	...
	$srch_parms = array();
    $srch_parms['srch_str'] = $jinput->post->get('srch_str','','STRING');
    $srch_parms['any_all'] = $jinput->post->get('any_all','','STRING');
    return $srch_parms;
...
function show_member_data($members)
{
    ...
    foreach ($members as $member) {
        ...
        if (empty($member->url_link)) {
                ...
                $url_link = '<a href="http://www.' . stristr($member->url_link, "linkedin.com") . ' ">' .
               '<img src="https://static.licdn.com/scds/common/u/img/webpromo/btn_liprofile_blue_80x15.png" ' .
               ' width="80" height="15" border="0"></a>'  ;
        } elseif (!stripos($member->url_link,"http") || stripos($member->url_link, "http") != 0) {
                $url_link = "<a href='http://" . $member->url_link . "'>" . $member->url_link . "</a>";
        } else {
                $url_link = "<a href='" . $member->url_link . "'>" . $member->url_link . "</a>";
        } 
...
{    
    ...
    echo '<br><table>';
    echo '<tr>';
    echo '<td align="right">Skills,&nbspexperience&nbspor&nbspkeyword&nbspneeded:</td>' .
    ...
    $checked = $srch_parms['any_all'] != "ALL" ? " checked" : "";
    echo '<td align="left"><input type="radio" name="any_all" ' .
        'value="ANY"' . $checked . '>  Match any listing containing a single specification</td>';
    echo '</tr>';
    echo '</table>';
...
function show_search_results($srch_parms)
{
    /*
...
// a fulltext search, create temporary MyISAM table with needed data 
  ...
  ->where($db->quoteName('em.profile') . ' IS NOT NULL AND ' .
  ...
  ->join('LEFT', $db->quoteName('eu_member_urls','eu') .
...
if (!$postdata->get('process') || $postdata->get('action') == 'Reset') {
	...
	show_search_form($srch_parms);
	show_search_results($srch_parms);
} else {	
	// how did we get here?
