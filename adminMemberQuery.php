 ...
 * 05/20/2015 bb	1.14	add comments, don't pass $db to functions
 * 04/15/2015 bb 1.13 	change date format, blank 0/0/00 dates, 
 * 				blank "Not Selected" and "Undecided" in table, darken "disabled" 
 * 				controls, change column order, change title "Jobclass" to "Job Class", 
 * 				add columns for city, state, and zip, format phone #s, add Back button, 
 * 				restyle table
 * 04/09/2015 bb 1.12.1	turn off email protector for the members table and 
 * 				revert ' AT ' to '@' change -- only admins have access to 
 *              this script anyway, so no need to cloak emails
 * 1.12 - bb, email protector still causing problems after update to 1.3.9, so 
 * 				replace @-sign in email address with ' AT ' string to hide 
 *          	email addresses from email protector. (ironic, isn't it.) need to 
 * 				contact NoNumber to resolve this, it seems...
 * 1.11 - bb, revert turn-off of output buffering (is now back on) - timeout was 
 * 				probably caused by NoNumber Email Protector plugin
 * 1.10 - bb, turn off output buffering (was creating 504 gateway timeout errors 
 * 				for large tables), blocksize to 5000, change styling of table output
 * 1.9 - rl, change default search to "All Selected" from "Active" 3/28/2015, 
 * 				search block to 500 (was 30) [bb]
 ...
 * 1.1 - bb, Use NoNumber email cloaking rather than Joomla! stock cloaking 
 * 				(which did not work)
 */
...
function resetSearchParms()
{
    ...
    // limit initial/default display to members with "All Selected" status 
    // (change from "Active" rl 3/28/2015 
    $srch_parms['status'] = "";
...
function showMemberData($members)
{
    ...
    foreach ($members as $member) {
        ...
        $member->active_date) . "</td>" .
        "<td>" . $_ = ($member->inactive_date=='00/00/0000' ? '' : 
        $member->inactive_date) . "</td>" .
        "<td>" . $_ = ($member->orient_date=='00/00/0000' ? '' : 
        $member->orient_date) . "</td>" .
...
{    
    ...
    echo '<tr><td class="blabel">Active&nbspDate&nbsp-&nbspFrom</td>' .
        '<td style="text-align:left; width:70%;">' .
        '<input type="text" name="from_active_date" ' .
    ...
    echo '<tr><td class="blabel">Active&nbspDate&nbsp-&nbspTo</td>' .
        '<td style="text-align:left; width:70%;">' .
        '<input type="text" name="to_active_date" ' .
    ...
    echo '<tr><td class="blabel">Inactive&nbspDate&nbsp-&nbspFrom</td>' .
        '<td style="text-align:left; width:70%;">' .
        '<input type="text" name="from_inactive_date" ' .
    ...
    echo '<tr><td class="blabel">Inactive&nbspDate&nbsp-&nbspTo</td>' .
        '<td style="text-align:left; width:70%;">' .
        '<input type="text" name="to_inactive_date" ' .
    ...
    echo '<tr><td class="blabel">Orientation&nbspDate&nbsp-&nbspFrom</td>' .
        '<td style="text-align:left; width:70%;">' .
        '<input type="text" name="from_orient_date" ' .
    ...
    echo '<tr><td class="blabel">Orientation&nbspDate&nbsp-&nbspTo</td>' .
        '<td style="text-align:left; width:70%;">' .
        '<input type="text" name="to_orient_date" ' .
...
function showSearchResults($srch_parms)
{
    ...
    showMemberData($members, $mysession);
...
function buildQueries($srch_parms)
{
    ...
    $query
        ...
        ->select(
            'SUM(' . $db->quoteName('eh.task_hours') . ') AS ' . 
        ...
        ->join(
            'LEFT', $db->quotename('eu_member_hours', 'eh') . 
    ...
    if (!empty($srch_parms['from_active_date'])) {
        $query->where(
            $db->quoteName('em.active_date') . 
            ' >= STR_TO_DATE(' . $db->quote($srch_parms['from_active_date']) . 
            ',"%m/%d/%Y")'
        );
    }
    if (!empty($srch_parms['to_active_date'])) {
        $query->where(
            $db->quoteName('em.active_date') . 
            ' <= STR_TO_DATE(' . $db->quote($srch_parms['to_active_date']) . 
            ',"%m/%d/%Y")'
        );
    }
    if (!empty($srch_parms['from_inactive_date'])) {
        $query->where(
            ...
            ' >= STR_TO_DATE(' . $db->quote($srch_parms['from_inactive_date']) . 
            ',"%m/%d/%Y")'
        );
    }
    if (!empty($srch_parms['to_inactive_date'])) {
        $query->where(
            $db->quoteName('em.inactive_date') . 
            ' <= STR_TO_DATE(' . $db->quote($srch_parms['to_inactive_date']) . 
            ',"%m/%d/%Y")'
        );
    }
    if (!empty($srch_parms['from_orient_date'])) {
        $query->where(
            ...
            ' >= STR_TO_DATE(' . $db->quote($srch_parms['from_orient_date']) . 
            ',"%m/%d/%Y")'
        );
    }
    if (!empty($srch_parms['to_orient_date'])) {
        $query->where(
            $db->quoteName('em.orient_date') . 
            ' <= STR_TO_DATE(' . $db->quote($srch_parms['to_orient_date']) . 
            ',"%m/%d/%Y")'
        );
    }
