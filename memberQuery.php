 ...
 * 1.6 bb 04/15/15 -- change query results date format, blank 0 dates, blank "Not Selected" 
 * 				and "Undecided" in table, darken "disabled" controls, change table styling,
 * 				change default search status to AL, format all phone #s, added Back button
 ...
 * 1.2bhb -- added </form> tag, which had been missing
 * 1.1 - Use NoNumber email cloaking rather than Joomla! stock cloaking (which did not work)
 */
 
echo <<<EOS
<h2>   Member Query v1.6  </h2>
EOS;
...
function validated_date($date_strg)
{
	...
	if (empty($date_strg)) {
		...
		echo '<br><font color="red"><strong>Bad format for Active Date</strong></font><br>';
	}
...
function load_search_parms($postdata)
{
    ...
    $srch_parms['active_from_date'] = 
	    validated_date($postdata->get('active_from_date','','STRING'));
    $srch_parms['active_to_date'] = 
	    validated_date($postdata->get('active_to_date','','STRING'));
    $srch_parms['inactive_from_date'] = 
	    validated_date($postdata->get('inactive_from_date','','STRING'));
    $srch_parms['inactive_to_date'] = 
	    validated_date($postdata->get('inactive_to_date','','STRING'));
    $srch_parms['orient_from_date'] = 
	    validated_date($postdata->get('orient_from_date','','STRING'));
    $srch_parms['orient_to_date'] = 
	    validated_date($postdata->get('orient_to_date','','STRING'));
   return $srch_parms;
...
function show_member_data($members)
{
    ...
    echo '<tr><th>First Name</th><th>Last Name</th>' .
        '<th>Home Phone</th><th>Mobile Phone</th><th>Email</th><th>Industry</th>' .
        '<th>Job Class</th><th>Status</th><th>Committee</th><th>Active Date</th></tr>';
    ...
    foreach ($members as $member) {
		...
		"<td>" . $_ = $member->active_date=='00/00/0000' ? '' : $member->active_date . "</td></tr>";
    }
...
{    
    ...
    echo '<tr><td class="blabel">Active&nbspDate&nbsp-&nbspFrom</td>' .
        '<td style="text-align:left; width:70%;"><input type="text" name="active_from_date" ' .
...
function show_search_results($db, $mysession, $srch_parms, $postdata)
{
    ...
    echo "<input type='hidden' name='jobclass_id' value=$srch_parms[jobclass_id]>";
    echo "<input type='hidden' name='industry_id' value=$srch_parms[industry_id]>";
    echo "<input type='hidden' name='committee' value=$srch_parms[committee]>";    
    echo "<input type='hidden' name='active_to_date' value=$srch_parms[active_to_date]>";
    echo "<input type='hidden' name='active_from_date' value=$srch_parms[active_from_date]>";
    echo "<input type='hidden' name='inactive_to_date' value=$srch_parms[inactive_to_date]>";
    echo "<input type='hidden' name='inactive_from_date' value=$srch_parms[inactive_from_date]>";
    echo "<input type='hidden' name='orient_to_date' value=$srch_parms[orient_to_date]>";
    echo "<input type='hidden' name='orient_from_date' value=$srch_parms[orient_from_date]>";
...
function buildQueries($db,$srch_parms)
{
	...
	$query
		...
		->where($db->quoteName('ei.industry_id') . 
		' = ' . $db->quoteName('em.industry_id'))
		->where($db->quoteName('ej.jobclass_id') . 
		' = ' . $db->quoteName('em.jobclass_id'))
		->where($db->quoteName('em.status') . ' <> "D"')
		->where($db->quoteName('es.member_status') . 
		' = ' . $db->quoteName('em.status'))
		->where($db->quoteName('ec.committee') . 
	...
	if (!empty($srch_parms['active_from_date'])) {
		...
		' >= STR_TO_DATE(' . $db->quote($srch_parms['active_from_date']) . ',"%m/%d/%Y")');
	}
	if (!empty($srch_parms['active_to_date'])) {
		$query->where($db->quoteName('em.active_date') . 
		' <= STR_TO_DATE(' . $db->quote($srch_parms['active_to_date']) . ',"%m/%d/%Y")');
	}
	if (!empty($srch_parms['inactive_from_date'])) {
		$query->where($db->quoteName('em.inactive_date') . 
		' >= STR_TO_DATE(' . $db->quote($srch_parms['inactive_from_date']) . ',"%m/%d/%Y")');
	}
	if (!empty($srch_parms['inactive_to_date'])) {
		$query->where($db->quoteName('em.inactive_date') . 
		' <= STR_TO_DATE(' . $db->quote($srch_parms['inactive_to_date']) . ',"%m/%d/%Y")');
	}
	if (!empty($srch_parms['orient_from_date'])) {
		$query->where($db->quoteName('em.orient_date') . 
		' >= STR_TO_DATE(' . $db->quote($srch_parms['orient_from_date']) . ',"%m/%d/%Y")');
	}
	if (!empty($srch_parms['orient_to_date'])) {
		$query->where($db->quoteName('em.orient_date') . 
		' <= STR_TO_DATE(' . $db->quote($srch_parms['orient_to_date']) . ',"%m/%d/%Y")');
	}
