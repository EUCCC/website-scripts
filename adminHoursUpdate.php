 ...
 * 
 * @package    adminHoursUpdate_v1.3.php
...
/* Modification log:
 * -------------------------------------------------------------------------
 * 2015/04/15	bb	 v1.4 renumber tasks starting at #1, change Enter to Other
 * 						added Back buttons (x2)
 * 2015/01/26	bb   v1.3 use database column to determine task display format
 * 2015/01/09	bb   v1.2 change to radio button format, set database
 ...
 *						tables
 * 2014/09/30   js   v1.0 initial release of Admin Volunteer Hours Entry
 */

echo <<<EOS
<h2>   Admin Hours Update v1.4  </h2>
EOS;
...
#------------------------------------------------------------------------
function show_hours_entry_form($db ,$member)
{
	...
	foreach ($taskArray['checkboxes'] as $task)	{
	    echo "<tr><td class='blabel'>{$task->task_name} (" . intval($task->default_task_hrs) . "&nbsph)</td>" .
...
#------------------------------------------------------------------------
function add_hours_and_update_database_tables($db)
{
	$totalHrsAdded = 0;	
...
if  (!isset($_POST['process'] )) { 
	...
	if (!isset($_POST['member_radio'])) {
		...
		show_hours_entry_form($db, $member);
	}
} elseif ($_POST['process'] == 4) { 
	add_hours_and_update_database_tables($db);
} else {
